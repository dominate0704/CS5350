...
%% OVER_TREE one is svm_over_tree, one is LR_over_tree
clear;
load('data.mat');
data{1} = training00;
data{2} = training01;
...
x1 = ['For SGD over tree, the best hyper_parameter: gamma = ', num2str(gamma_SGD), ' ; C = ', num2str(C_SGD)];
x2 = ['Average cross-validation accuracy = ',num2str(best/5)];
disp(x1);
disp(x2);
...
x3 = ['For SGD over tree, the Training Accuracy = ', num2str(correctNumber/length(traindata))];
[correctNumber,~] = checkErrors_form2(testdata,Answer_tree, w_cur);
TestAccuracyBG(1) = correctNumber/length(testdata);
x4 = ['For SGD over tree, the Testing Accuracy = ', num2str(correctNumber/length(testdata))];
disp(x3);
disp(x4);
...
x1 = ['For LR over tree, the best hyper_parameter: gamma = ', num2str(gamma_LR), ' ; sigma2 = ', num2str(sigma2_LR)];
x2 = ['Average cross-validation accuracy = ',num2str(best/5)];
disp(x1);
disp(x2);
svm_data = SVMovertrees(Answer_tree, traindata);
...
x3 = ['For LR over tree, the Training Accuracy = ', num2str(correctNumber/length(traindata))];
[correctNumber,~] = checkErrors_form2(testdata, Answer_tree, w_cur);
TestAccuracyBG(2) = correctNumber/length(testdata);
x4 = ['For LR over tree, the Testing Accuracy = ', num2str(correctNumber/length(testdata))];
disp(x3);
disp(x4);
